...
env_update_multiple(DB_ENV *env, DB *src_db, DB_TXN *txn,
...

    {
        ...
        if (r == 0 && n_del_dbs > 0) {
            ...
            r = do_del_multiple(txn, n_del_dbs, del_dbs, del_key_arrays, src_db, old_src_key, false);
        }
        ...
        if (r == 0 && n_put_dbs > 0) {
            ...
            r = do_put_multiple(txn, n_put_dbs, put_dbs, put_key_arrays, put_val_arrays, src_db, new_src_key, false);
        }
        toku_multi_operation_client_unlock();
        if (indexer_lock_taken) {
            toku_indexer_unlock(indexer);
